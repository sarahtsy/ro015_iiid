...
def main():
    ...
    with tf.Session() as sess:
        ...
        #sess is good actor, bad_sess is bad actor
        bad_sess = tf.Session()
        bad_saver = tf.train.Saver()
        ...
        for istep in range(config.TRAIN_STEPS):
            ...
            for i in range(accumulation_steps):
                ...
                bad_a_np, bad_out = bad_sess.run([a, accumulate_ops], feed_dict={s:s_np, g: g_np})
                a_np[:num_bad], out[:num_bad] = bad_a_np[:num_bad], bad_out[:num_bad]
