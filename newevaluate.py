...
def parse_args():
    ...
    parser.add_argument('--num_bad', type=int, required=True)
...
def main():
    ...
    # initialize the bad actors' tensorflow Session
    bad_sess = tf.Session()
    bad_sess.run(tf.global_variables_initializer())
    bad_saver = tf.train.Saver(var_list=vars_restore)
    bad_saver.restore(bad_sess, "models/bad_actors")
    ...
    for istep in range(config.EVALUATE_STEPS):
        ...
        for i in range(config.INNER_LOOPS):
            ...
            # !! overwrite actions with num_bad_agents' actions
            bad_a_network, bad_acc_list_np = bad_sess.run([a, acc_list], feed_dict={s: s_np, g: g_np})
            a_network[:num_bad] = bad_a_network[:num_bad]
            acc_list_np[:num_bad] = bad_acc_list_np[:num_bad]
        ...
        if args.vis:
            ...
            for j in range(max(len(s_np_ours), len(s_np_lqr))):
                ...
                plt.scatter(s_np[:num_bad, 0], s_np[:num_bad, 1], 
                            color='black', 
                            s=agent_size, label='Bad', alpha=0.6)
                plt.scatter(s_np[num_bad:, 0], s_np[num_bad:, 1], 
                            color='darkorange', 
                            s=agent_size, label='Agent', alpha=0.6)
                plt.scatter(g_np[:, 0], g_np[:, 1], 
                            color='deepskyblue', 
                            s=agent_size, label='Goal', alpha=0.6)
